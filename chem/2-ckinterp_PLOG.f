...
!     9          KORD(MAXORD,IDIM), RORD(MAXORD,IDIM)
 ...
 !    &          IFT1(IDIM), PFT1(NF1R,IDIM), IEXC(IDIM), PEXC(IDIM)
	...
	NARG=IARGC();
	IF(NARG>=2)THEN
	  CALL GetARG(1,ARGSTR1); !Inp 文件名
	  CALL GetARG(2,ARGSTR2); !Bin 文件名
	END IF
...
!     &     PFT1/NF1IDIM*0.0/, PEXC/IDIM*0.0/
	...
	!=================================================
      MM=0;KK=0;II=0; !重新复位  ··必须，因为MM/KK/II要进行累加
...
C*****END precision > single
  ...
  105 CONTINUE
      ...
      ILEN=IPPLEN(LINE);                 !获取有效长度
	IF(ILEN.EQ.0) GOTO 100;

      CALL CKISUB(LINE(:ILEN),SUB,NSUB); !取得各个部分信息
  ...
  312       CONTINUE
     ...
     &                  KCHRG,NT,T,TLO,TMID,THI,KNCF,ITHRM,LOUT,KERR);
      ...
      IF(ITASK.EQ.1)THEN !ELEMENT DATA
         ...
         IF(NSUB.GT.0)THEN
           M1=MM+1;
           CALL CKCHAR(SUB,NSUB,MDIM,ENAME,AWT,MM,KERR,LOUT); !获取到Element数量和对应的元素
...
C
  ...
  105 CONTINUE
	...
	IF(ILEN.EQ.0) GOTO 100;
      ...
      !====根据ITSAK进行对应处理   
      IF(ITASK.EQ.1)THEN !ELEMENT DATA
        !IF(NSUB.GT.0)CALL CKCHAR(SUB,NSUB,MDIM,ENAME,AWT,MM,KERR,LOUT); !获取到Element数量和对应的元素
	   IF(NSUB.GT.0)CALL CKCHAR_GOU(SUB,NSUB,MM,KERR,LOUT); !获取到Element数量和对应的元素
      ELSEIF(ITASK.EQ.2)THEN !PROCESS SPECIES DATA
        !IF(NSUB.GT.0)CALL CKCHAR(SUB,NSUB,KDIM,KNAME,WTM,KK,KERR,LOUT);!SPECIES 
	  ...
	  IF(IND.LE.0) II=II+1; !计算Reaction数量
      ENDIF
      GOTO 100
