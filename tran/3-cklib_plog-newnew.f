! This file was additionally modified by M P Burke (11-11-11) to improve one or two aspects
! of the PLOG implementation of Guo, Miller, Sun and Ju (2011).
!     All changes by M P Burke are labeled with comments
!
! This Cklib_PLOG.f is to replace ckinlib.f in Chemkin II and III for PLOG functions used in unamolecular reactions
!
...
C
C     PLOG function modification by Xiaolong Gou/Yiguang Ju (Princeton EFRC)
C     
C      To use PLOG function in this file, in the main Fortran code, use
c
c   ** Call _PLOG(LENI, LENR, LENC, LINC, Lout, ICKWRK, RCKWRK,CCKWRK)
C	                                                                                  C
C---------------------------------------------------------------------------------------C
C                                                                                       C
      SUBROUTINE CKABS
...
C     1. Implement Landau-Teller rate ression
...
C
      ...
      INCLUDE 'ckstrt.h'
C
      CALL CKRATT_PLOG (RCKWRK, ICKWRK, NII, MXSP, RCKWRK(NcRU),
...
C
  ...
  475 FORMAT (10X,'CHARACTER LENGTH OF CCKWRK MUST BE AT LEAST 16 ')
      ...
      SUBROUTINE CKINIT_PLOG(LENIWK,LENRWK,LENCWK,LINC,LOUT,ICKWRK,
     &                       RCKWRK,CCKWRK)
C
C  START PROLOGUE
C
...
C
     ...
     3   L = 1, MXORD), N=1, NORD)
      ...
      !--Read PLOG data  ``
	READ(LINC,END=100)I_PLOG(1:II+1);         !Start Pos. is ITOT+1
	READ(LINC,END=100)I_PLOG(II+1+1:II+II+1); !The Pressure point #
	DO N=1,II
	  K1=(N-1)*I_PLOG(1)*4+1;
	  READ(LINC,END=100)(R_PLOG(K1+K2-1),K2=1,I_PLOG(1)*4);
	END DO
C
  100 CONTINUE
      RETURN
...
C
 ...
 105  CONTINUE
      ...
      SUBROUTINE CKRATT_PLOG(RCKWRK,ICKWRK,II,MAXSP,RU,PATM,T,NSPEC,
     1                   NU, NUNK, NPAR, PAR, NREV, IREV, RPAR, NLAN,
     2                   NLAR, ILAN, PLT, NRLT, IRLT, RPLT, SMH, NRNU,
     3                   IRNU, RNU, NEIM, IEIM, ITDEP, NJAN, NJAR, IJAN,
     4                   PJAN, NFT1, NF1R, IFT1, PF1, RKFT, RKRT, EQK)
...
C
 ...
 103     CONTINUE
         ...
         RKFT(I) = MIN(BIG, RKFT(I) * EXP(SUMJ))
 105  CONTINUE
C	write(*,*) 'The regular rate constant of reaction', N, ' is:', RKFT(N)
C    	 CALL CALK_PLOG(RCKWRK,ICKWRK,KK,II,RU,PATM,T,RKFT);
C *** M P Burke 11-8-11
      CALL CALK_PLOG(RCKWRK,ICKWRK,KK,II,RU,PATM,T,NSPEC,RKFT,RKRT,EQK)
      RETURN
      END

	!-------------------------------------------------------
	!Subroutine CALK_PLOG 
	! Calculate K based on PLOG
	!------------------------------------------------------
C	SUBROUTINE CALK_PLOG(RCKWRK,ICKWRK,KK,II,RU,PATM,T,RKFT)
C *** M P Burke 11-8-11
       SUBROUTINE CALK_PLOG(RCKWRK,ICKWRK,KK,II,RU,PATM,
     &                     T,NSPEC,RKFT,RKRT,EQK)
      USE VAR_PLOG
      IMPLICIT DOUBLE PRECISION(A-H,O-Z),INTEGER(I-N)
      INCLUDE 'ckstrt.h'
C      DIMENSION RCKWRK(*),ICKWRK(*),T(*),RKFT(*)
C *** M P Burke 11-8-11
      DIMENSION RCKWRK(*),ICKWRK(*),T(*),NSPEC(*),RKFT(*),RKRT(*),EQK(*)
      COMMON /MACH/ SMALL,BIG,EXPARG
      ...
      PinATMnew=P/PATM;
C	   write(*,*) PinATMnew
	If(abs(PinATMnew-PinATM).gt.0.0001) then
C	   write(*,*) PinATM
C	   write(*,*) PinATMnew
	   write(*,*) 'Wrong pressure input in cklib_plog, stop'
	   stop
	end if
      XRUinJ=8.314510;
      XCALTOJ=4.184;
	...
	DO N=1,II
	  ...
	  IF(I_PLOG(N+1)>0)THEN   
	    
          K1=(N-1)*K0*4+1;      

			 
C****start***modified by M P Burke to improve interpolation implementation
          IFLAG1=0;
          IFLAG2=0;
          IS1=1;
          XTEQ=0;
          DO WHILE (IS1<=K0)          
          	DO WHILE ((ABS(PinATM-R_PLOG(K1+4*(IS1-1)))/PinATM)
     &         <PCUTOFFREL)                               
			XP=R_PLOG((IS1-1)*4+K1);
			XA=R_PLOG((IS1-1)*4+K1+1);
	    	XN=R_PLOG((IS1-1)*4+K1+2);
			XE=R_PLOG((IS1-1)*4+K1+3);
			XEinJ=XE*XCALTOJ;
		    XTEQ=XTEQ+XA*T(1)**XN*DEXP(-XEinJ/(XRUinJ*T(1)));!

			IS1=IS1+1;
			IFLAG1=1;									
          	END DO

          	IF(PinATM>R_PLOG(K1+4*(IS1-1))
     &         .AND.PinATM<R_PLOG(K1+4*(IS1))) THEN
			XPL=R_PLOG(K1+4*(IS1-1));
			XPH=R_PLOG(K1+4*(IS1));
			IFLAG2=1;									
		    END IF
		    IS1=IS1+1;
          END DO
C            write(*,*) iflag1, iflag2
C			write(*,*) R_PLOG(K1+4*(IS1-1)), R_PLOG(K1+4*(IS1)), PinATM
			
   
          IF(IFLAG1==1) THEN                              
          	RKFT(N)=XTEQ;
          END IF
          IF(IFLAG1==0.AND.IFLAG2==1) THEN				
		IS1=1;
		XTL=0;
		XTH=0;
          	DO WHILE (IS1<=K0)
          		IF((ABS(XPL-R_PLOG(K1+4*(IS1-1)))/XPL)<PCUTOFFREL) THEN
              		XA=R_PLOG((IS1-1)*4+K1+1); !
	        		XN=R_PLOG((IS1-1)*4+K1+2); !
	        		XE=R_PLOG((IS1-1)*4+K1+3); !
	        		XEinJ=XE*XCALTOJ;
		    		XTL=XTL+XA*T(1)**XN*DEXP(-XEinJ/(XRUinJ*T(1)));!

			END IF
          		IF((ABS(XPH-R_PLOG(K1+4*(IS1-1)))/XPH)<PCUTOFFREL) THEN
              		XA=R_PLOG((IS1-1)*4+K1+1); !
	        		XN=R_PLOG((IS1-1)*4+K1+2); !
	        		XE=R_PLOG((IS1-1)*4+K1+3); !
	        		XEinJ=XE*XCALTOJ;
		    		XTH=XTH+XA*T(1)**XN*DEXP(-XEinJ/(XRUinJ*T(1)));!

			END IF
			IS1=IS1+1;
		END DO
		RKFT(N)=EXP(LOG(XTL)+(LOG(PinATM)-LOG(XPL))*
     &	     (LOG(XTH)-LOG(XTL))/(LOG(XPH)-LOG(XPL)));
          END IF
		  
		  
          IF(IFLAG1==0.AND.IFLAG2==0) THEN
		  
C          	WRITE(*,*) 'ERROR: SYSTEM PRESSURE IS OUTSIDE',
C     &	          ' PLOG VALUES. CALCULATION TERMINATED'
C          	WRITE(*,*) 'ERROR: SYSTEM PRESSURE IS OUTSIDE',
C     &	          ' PLOG VALUES. CALCULATION TERMINATED'
C	 	    STOP
	 
C****start***modified by Fujia Wu to handle the case when system Pressure
C************is outside PLOG values	 
	 
C          	WRITE(*,*) 'WARNING: SYSTEM PRESSURE IS OUTSIDE',
C     &	          ' PLOG VALUES

			XTOK = 0
            if (PinATM < R_PLOG(K1)) then
			
			XA=R_PLOG(K1+1);
	    	XN=R_PLOG(K1+2);
			XE=R_PLOG(K1+3);
			XEinJ=XE*XCALTOJ;
		    XTOK=XTOK+XA*T(1)**XN*DEXP(-XEinJ/(XRUinJ*T(1)));!
			
            end if
			
           if (PinATM > R_PLOG(K1+(I_PLOG(N+1)-1)*4) ) then
		   
			XA=R_PLOG(K1+(I_PLOG(N+1)-1)*4+1);
	    	XN=R_PLOG(K1+(I_PLOG(N+1)-1)*4+2);
			XE=R_PLOG(K1+(I_PLOG(N+1)-1)*4+3);
C			write(*,*) XA, XN, XE
			XEinJ=XE*XCALTOJ;
		    XTOK=XTOK+XA*T(1)**XN*DEXP(-XEinJ/(XRUinJ*T(1)));!		
...
C****end***modified by Fujia Wu to handle the case when system Pressure
C************is outside PLOG values	 
			
          END IF
		  
		  
C****end***modified by M P Burke to improve interpolation implementation

C****start***added by M P Burke to calculate reverse rate constant
C            for reversible reactions (NSPEC(I).GT.0)
		  IF (NSPEC(N).GT.0) RKRT(N) = RKFT(N) / MAX(EQK(N),SMALL);  !return backward reaction rates with the input of equilibrium constant
C****end***added by M P Burke to calculate reverse rate constant
